...
async function connectUserIMChannels(
  ...
  if (telegramConfig && telegramConfig.enabled !== false && telegramConfig.botToken) {
    const isChatAuthorized = (jid: string) => {
      const group = registeredGroups[jid];
      return !!group && group.created_by === userId;
    };
    const onPairAttempt = async (jid: string, chatName: string, code: string): Promise<boolean> => {
      const result = verifyPairingCode(code);
      if (!result) return false;
      // Only allow pairing on this bot if the code was generated by the bot owner
    ...
    telegram = await imManager.connectUserTelegram(userId, telegramConfig, onNewChat, isChatAuthorized, onPairAttempt);
  }
...
async function main(): Promise<void> {
  ...
  const reloadTelegramConnection = async (config: { botToken: string; enabled?: boolean }): Promise<boolean> => {
    ...
    if (config.enabled !== false && config.botToken) {
      ...
      const isChatAuthorized = (jid: string) => {
        const group = registeredGroups[jid];
        return !!group && group.created_by === adminUser.id;
      };
      const onPairAttempt = async (jid: string, chatName: string, code: string): Promise<boolean> => {
        const result = verifyPairingCode(code);
        if (!result) return false;
        if (result.userId !== adminUser.id) return false;
        const pairingUserHome = getUserHomeGroup(result.userId);
        if (!pairingUserHome) return false;
        buildOnNewChat(result.userId, pairingUserHome.folder)(jid, chatName);
        return true;
      };
      const connected = await imManager.connectUserTelegram(adminUser.id, config, onNewChat, isChatAuthorized, onPairAttempt);
      return connected;
    }
  ...
  const reloadUserIMConfig = async (userId: string, channel: 'feishu' | 'telegram'): Promise<boolean> => {
    ...
    if (channel === 'feishu') {
      ...
      if (config && config.enabled !== false && config.botToken) {
        ...
        const connected = await imManager.connectUserTelegram(userId, config, onNewChat, isChatAuthorized, onPairAttempt);
