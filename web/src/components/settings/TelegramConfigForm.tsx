...
export function TelegramConfigForm({ setNotice, setError }: TelegramConfigFormProps) {
  ...
  const handleToggle = async (newEnabled: boolean) => {
    ...
    setError(null);
    try {
      const saved = await api.put<TelegramConfigPublic>('/api/config/telegram', {
        enabled: newEnabled,
      });
      setConfig(saved);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (proxyUrl.trim()) payload.proxyUrl = proxyUrl.trim();
      if (!proxyUrl.trim() && clearProxyUrl) payload.clearProxyUrl = true;

      await api.put<TelegramConfigPublic>('/api/config/telegram', payload);
      ...
      setBotToken('');
      setClearToken(false);
      setProxyUrl(fresh.proxyUrl || '');
      setClearProxyUrl(false);
      setEnabled(fresh.enabled);
      setNotice(
        `Telegram 配置已保存${fresh.connected ? '并已连接' : ''}（当前代理：${fresh.proxyUrl || '直连'}）`,
      );
    } catch (err) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div
        className={`px-5 py-4 space-y-4 transition-opacity ${formDisabled ? 'opacity-50 pointer-events-none' : ''}`}
      >
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">
            在 Telegram 中搜索 @BotFather，发送 /newbot 创建机器人后获得
          </p>
        ...
        <div>
          <label className="block text-xs text-slate-600 mb-1">代理 URL</label>
          <Input
            type="text"
            value={proxyUrl}
          ...
          />
          <p className="mt-1 text-xs text-slate-400">
            不填则直连 Telegram；填写后测试连接与运行时都会通过该代理。
          </p>
          <label className="mt-2 flex items-center gap-2 text-xs text-slate-600">
            <input
              type="checkbox"
              checked={clearProxyUrl}
              onChange={(e) => setClearProxyUrl(e.target.checked)}
              disabled={saving}
            />
            清空代理 URL
          </label>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button
            variant="outline"
            onClick={handleTest}
            disabled={loading || testing || !config?.hasBotToken}
          >
        ...
        <div className="text-xs text-slate-500 space-y-1">
          ...
          <div>当前代理：{config?.proxyUrl || '直连'}</div>
          <div>
            最近保存：{config?.updatedAt ? new Date(config.updatedAt).toLocaleString('zh-CN') : '未记录'}
          </div>
        </div>
      </div>
    </div>
