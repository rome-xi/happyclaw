...
const FOLDER_SUB_GROUPED: Set<MemoryType> = new Set([
  'session',
  'date',
  'conversation',
]);
  ...
  return (
    ...
    >
      ...
      <div className="text-[11px] mt-1 text-muted-foreground">
        {source.writable ? '可编辑' : '只读'} ·{' '}
        {source.exists ? `${source.size} B` : '文件不存在'}
      </div>
      {hit && (
...
export function MemoryPage() {
  ...
  const filteredSources = useMemo(() => {
    ...
    return sources.filter(
      (s) =>
        `${s.label} ${s.path}`.toLowerCase().includes(text) ||
        Boolean(searchHits[s.path]),
    );
  ...
  const loadSources = useCallback(async () => {
    ...
    try {
      const data = await api.get<{ sources: MemorySource[] }>(
        '/api/memory/sources',
      );
      ...
      let nextSelected =
        selectedPath && available.has(selectedPath) ? selectedPath : null;
        ...
        if (folderParam) {
          nextSelected =
            data.sources.find(
              (s) =>
                s.type === 'session' &&
                s.path.includes(`/${folderParam}/`) &&
                s.path.endsWith('CLAUDE.md'),
            )?.path || null;
        }
      }
      ...
      if (!nextSelected) {
        ...
        nextSelected =
          ...
          data.sources.find(
            (s) => s.type === 'session' && s.path.endsWith('CLAUDE.md'),
          )?.path ||
          data.sources[0]?.path ||
          null;
      }
                    ...
                    (extResources.rules.length > 0 ||
                      ...
                      <div>
                        ...
                        >
                          ...
                          宿主机配置 (
                          {extResources.rules.length +
                            (extResources.claudeMd ? 1 : 0)}
                          )
                        </button>
                        {!extCollapsed && (
                          <div className="space-y-1 ml-1">
                            <div className="text-[11px] text-amber-700 dark:text-amber-500 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50 rounded-lg px-2.5 py-1.5 mb-1">
                              ⚠️ 宿主机模式下，这里的 CLAUDE.md / rules
                              会直接写回本机{' '}
                              <code className="font-mono">~/.claude/</code>
                              ，并影响这台机器上的其他 Claude Code 项目。
                            </div>
                            {extResources.claudeMd && (
                              <button
                                onClick={() => {
                                  ...
                                  setFileMeta({
                                    path: 'ext:CLAUDE.md',
                                    content: extResources.claudeMd!,
                                    updatedAt: null,
                                    size: extResources.claudeMd!.length,
                                    writable: false,
                                  });
                                  if (isMobile) setShowContent(true);
                                }}
                                className={`w-full text-left rounded-lg border px-3 py-2 transition-colors ${
                                  selectedPath === 'ext:CLAUDE.md'
                                    ? 'border-primary bg-brand-50'
                                    : 'border-border hover:bg-muted/50'
                                }`}
                              >
                                ...
                                <div className="text-[11px] text-muted-foreground mt-0.5">
                                  {extResources.dir}/CLAUDE.md
                                </div>
                                <div className="text-[11px] mt-1 text-muted-foreground">
                                  只读 · {extResources.claudeMd.length} B
                                </div>
                              </button>
                            )}
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    const res = await fetch(
                                      `/api/config/external-resources/rule?name=${encodeURIComponent(r.name)}`,
                                      { credentials: 'include' },
                                    );
                                    const text = res.ok
                                      ? await res.text()
                  ...
                  <>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        最近更新时间: {updatedText} · 字节数:{' '}
                        {new TextEncoder().encode(content).length} ·{' '}
                        {fileMeta?.writable ? '可编辑' : '只读'}
                      </div>
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      <Button
                        onClick={handleSave}
                        disabled={
                          loadingFile || saving || !fileMeta?.writable || !dirty
                        }
                      >
