...
- 每个候选必须在 `src/tier-catalog.ts` 登记上下文长度；`max` 强制只接受至少 1M 上下文的模型。
...
| 源渠道名                 | 至少包含的模型                                                  |
| ------------------------ | --------------------------------------------------------------- |
| `super-relay (字节内部)` | 由 `config/model-routing.json` 中同名 source 的启用模型自动同步 |
...
模型目录、别名、协议、上下文长度、源渠道与四档候选全部位于
`config/model-routing.json`，文件不含上游密钥或账户凭据。新增模型只需增加一条 `models`
配置；如需参加自动选优，再把其 ID 加入对应 `tiers.*.models`。`syncModels=true` 的源渠道
会由内置探针把配置中的模型增量同步到 new-api（保留频道里其他手工模型），无需再到 UI
重复登记。未知模型不会自动进入任何档位，
避免弱模型悄悄混入 `max`，但配置后可立即用 `/model <模型或别名>` 手动选择。

档位别名使用所有候选中的最小上下文窗口，确保探针切换或请求回退后不会超过新上游容量。
`max` 声明 `minimumContextWindowTokens=1000000`：只要误加低于 1M 的候选，配置校验和测试
都会失败。TypeScript 探针与 Python gateway 都按 mtime 热加载同一配置；无效编辑会保留最后
一份有效配置。新增模型时必须先核实 `contextWindowTokens`，没有官方元数据时宁可保守填写。
...
Web、飞书与 Telegram 等 IM 使用同一组工作区级命令：

```text
/model                 # 查看当前选择
/model list            # 查看档位、真实模型、短别名、上下文与特征标签
/model grok            # 用别名固定到 grok/grok-4.5
/model high            # 固定到 high，但档内仍由探针实时选优
/model auto            # 清除手动选择，恢复 provider 默认路由
```

切换会停止该工作区正在运行的任务、丢弃所有旧 SDK session，并在下一条消息从 HappyClaw
数据库恢复最近历史，以避免不同模型/provider 的 thinking 签名混用产生 400。命令只保存公开
模型 ID 到本地数据库，不保存或回显任何上游密钥。
...
5. 在 Web 与 IM 分别执行 `/model grok`、`/model status`、`/model auto`，确认回执一致且切换后普通消息成功。
6. 请求 Agent 生成并发送图片，确认收到真实图片文件。
7. 新建工作区和定时 Agent 任务，UI 应只显示“宿主机（固定）”，系统不创建 HappyClaw Agent 容器。
