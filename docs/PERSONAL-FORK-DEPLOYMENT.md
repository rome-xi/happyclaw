...
每个真实模型都必须在 `src/tier-catalog.ts` 显式登记 `contextWindowTokens`。档位别名使用
所有可能候选中的最小上下文窗口，确保探针切换或请求回退后不会超过新上游的容量。
`max` 还声明了 `minimumContextWindowTokens=1000000`：只要误加低于 1M 的候选，启动校验和
测试都会直接失败；因此 `max` 自动使用 1M，而 `high/balance/fast` 仍按各自候选的安全
公共上限运行。新增模型时先确认真实上下文长度并登记，再把它加入相应档位。
