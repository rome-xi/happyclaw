        ...
        try:
            ...
            if m:
                ...
                if v and v not in keys:
                    keys.append(v)
            # 备用 key
            m2 = re.search(r"^\s*TAVILY_API_KEY_BACKUP\s*=\s*(.+?)\s*$", txt, re.M)
            if m2:
    ...
    for idx, key in enumerate(keys):
        ...
        try:
            ...
            try:
                ...
                last_error = f"Tavily returned non-JSON: {body[:300]}"
                if idx == len(keys) - 1:
                    raise SystemExit(last_error)
                continue
        except urllib.error.HTTPError as e:
            # 432 = 限流 / 配额耗尽,尝试下一个 key
            if e.code in (432, 429) and idx < len(keys) - 1:
                last_error = f"Key {idx} HTTPError {e.code}, trying next key"
                continue
            else:
                if idx == len(keys) - 1:
                    raise
        ...
        raise SystemExit(f"All Tavily keys exhausted: {last_error}")
